...
osgQtWidget::osgQtWidget(QWidget* parent, const QGLWidget* shareWidget, Qt::WindowFlags f, bool forwardKeyEvents)
...
osgQtWidget::osgQtWidget(QGLContext* context, QWidget* parent, const QGLWidget* shareWidget, Qt::WindowFlags f,
...
osgQtWidget::osgQtWidget(const QGLFormat& format, QWidget* parent, const QGLWidget* shareWidget, Qt::WindowFlags f,
	bool forwardKeyEvents)
...
void osgQtWidget::setTouchEventsEnabled(bool e)
{
#ifdef USE_GESTURES
...
void osgQtWidget::processDeferredEvents()
{
...
bool osgQtWidget::event(QEvent* event)
{
#ifdef USE_GESTURES
	...
	// QEvent::ParentChange
	//
	// Reparenting osgQtWidget may create a new underlying window and a new GL context.
...
bool osgQtWidget::gestureEvent(QGestureEvent* qevent)
{
#ifndef USE_GESTURES
	return false;
#else
...
GraphicsWindowQt::GraphicsWindowQt(osgQtWidget* widget)
	: _realized(false)
{
	_widget = widget;
...
GraphicsWindowQt::~GraphicsWindowQt()
{
	...
	// remove reference from osgQtWidget
	if (_widget)
		_widget->_gw = NULL;
}
...
bool GraphicsWindowQt::init(QWidget* parent, const QGLWidget* shareWidget, Qt::WindowFlags f)
{
	...
	if (!_widget)
	{
		...
		// create widget
		_widget = new osgQtWidget(traits2qglFormat(_traits.get()), parent, shareWidget, flags);
	}
